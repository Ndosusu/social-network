    ...
    return (
        <div id="newPostModal" className="modal neon-xl bg-primaryT h-9/10 w-3/5 absolute z-10 inset-1/2 -translate-1/2 rounded-xl hidden">
            <form className="w-full h-full flex flex-col justify-between items-center p-7 gap-5" onSubmit={newPostResolve}>
                ...
                <CreateOptionList opts={["Public", "Followers only", "Private", "Group"]} />
...
function CreateOptionList({opts}) {
    return (
        <div className="w-full h-fit flex justify-between gap-5 text-center">
            {opts.map((obj, i) => <CreateOption text={obj} val={i+1} key={i} />)} 
        </div>
    )
...
function CreateOption({text, val}) {
    return (
        <label htmlFor={"radio"+val} className="neon-sm rounded-xl p-2 w-2/10">
            <input type="radio" name="radioPrivacy" id={"radio"+val} value={val} defaultChecked={val == 1 ? true : false} className="hidden" />
            <p>{text}</p>
        </label>
    )
}