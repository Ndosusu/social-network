...
export default function DetailPostModal() {
    ...
    const deletePost = () => {
        ...
        .then(response => {
            if(CheckApiResponse(response)){
                postList.val.splice(postList.val.indexOf(curPost.val), 1)
                postList.set(postList.val)
                modal.set("")
    ...
    const updatePostList = (post) => {
        const index = postList.val.indexOf(curPost.val)
        const copyList = structuredClone(postList.val)

        copyList[index] = post
        postList.set(copyList)
    }
