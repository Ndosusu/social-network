...
func ProfileFeedHandler(w http.ResponseWriter, r *http.Request) {
	...
	pdb := post.New(&db)
	result, err := pdb.GetProfileFeed(map[string]any{
	...
	if err != nil {
		utils.JSONResponse(w, http.StatusInternalServerError, "Failed to retrieve profile feed", nil)
		return
	}

	utils.JSONResponse(w, http.StatusOK, "Profile feed retrieved successfully", result)
}
