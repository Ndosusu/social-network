...
func (db *PostDB) GetProfileFeed(obj map[string]any) (*models.Response, error) {
	/*
		expected input (as json object) :
			{
				session_uuid : string,
	...
	*/
	stmt := `SELECT 
			...
			WHERE
				p.author_id = ?
				AND (
					(p.privacy_mode = 1 AND u.private_mode = 0)
					OR (f.user_to IS NOT NULL AND p.privacy_mode IN (1,2))
					OR (p.privacy_mode = 3 AND pr.follower_id IS NOT NULL)
					OR (p.group_id IS NOT NULL AND gmr.group_id IS NOT NULL)
				)	
			...
			ORDER BY p.id DESC
			LIMIT ?;`
	rows, err := db.Conn.Query(stmt, obj["session_uuid"], obj["user_id"], obj["last_id"], obj["limit"])
	if err != nil {
		return nil, err
	}
	defer rows.Close()
